...
async function submitAction() {
    ...
    try {
        ...
        const response = await apiFetch(`/api/reviews/${review.id}/${state.selectedAction}`, {
            method: "POST",
            headers: {
...
function renderReviewList() {
    ...
    for (const review of state.reviews) {
        ...
        button.append(line(review.memory.subject, "review-title"), line(`${review.memory.scopeType} / ${review.memory.memoryType} / ${review.memory.status}`, "review-meta"), line(shortDate(review.createdAt), "review-date"));
...
function renderDetail() {
    ...
    elements.detail.append(heading(review.memory.subject), paragraph(review.memory.object, "memory-object"), detailGrid(rows), paragraph(review.notes ?? "", "notes"));
}
...
function resetActionDefaults(review) {
