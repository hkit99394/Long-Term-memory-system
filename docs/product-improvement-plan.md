...
| REG-01 | P0 | Done | Product Owner + Security Professional + Knowledge Steward + Developer | Project Registration Specification And Dry-Run Contract | [Project Registration UX Plan](project-registration-ux-plan.md) defines the REG-01 dry-run contract inside `scripts/project-onboarding-runbook.sh --dry-run`, covering required fields, validation rules, least-privilege presets, source-doc checks, access preview, audit evidence, planned operations, and closeout criteria. |
| REG-02 | P0 | Done | Developer + Security Professional | First Project Registration API Contract | `POST /api/admin/projects/register` adds the payload-safe, idempotent first registration API for organization/project upsert, project role definitions, owner memberships, role assignments, namespace grants, and `project_registration` audit evidence. |
| REG-03 | P0 | Done | Designer + Developer | Admin Wizard MVP | `/admin/` now includes a Project Registration mode with Project Details, People & Responsibilities, Seed Evidence, Access Rules, Review & Validate, Register, and Finish Setup steps; the wizard validates least-privilege inputs, runs effective-access checks, and submits `POST /api/admin/projects/register` with `Idempotency-Key`. |
| REG-04 | P0 | Done | Security Professional + Ops | Least-Privilege Bootstrap Cleanup | Normal registration and namespace grants now stay on read/write/review presets; break-glass namespace admin is a separate Access Management path that requires a single principal target, owner/accepted-by roles, reason, review due date, cleanup action, audit evidence id, and payload-safe audit metadata. |
| REG-05 | P1 | Done | Knowledge Steward + Product Owner | Source-Backed Seed Readiness UX | The Project Registration wizard now shows path coverage, SHA-256 fingerprint coverage, source owners, canonical evidence type coverage, role-specific guidance for owner responsibilities, retrieval check status, and feedback closeout status without collecting raw source payloads. |
| REG-06 | P1 | Done | Product Owner + Tester/QA | Registration Success Benchmark | The Project Registration wizard now records payload-safe success evidence for duration, blocked validation attempts, access changes from plan, source evidence coverage, and user confidence, with matching fields in the project-success scorecard, weekly-cycle, and closeout templates. |
| OPM-01 | P0 | Done | Product Owner + Developer + Security Professional | Admin Organization/Project Read Model | [Organization And Project Management OPM-01](organization-project-management-opm01.md) adds payload-safe, access-filtered list/detail endpoints for organizations and projects with pagination, counts, statuses, and latest registration evidence. |
| OPM-02 | P0 | Done | Product Owner + Designer + Developer | Admin Management UI Shell | [Organization And Project Management OPM-02](organization-project-management-opm02.md) adds the `/admin/` Management view for Organizations and Projects, backed by OPM-01 endpoints with search, project status filtering, payload-safe detail evidence, and empty/loading/error/not-visible states. |
| OPM-03 | P0 | Done | Product Owner + Security Professional + Developer | Lifecycle And Scope Settings | [Organization And Project Management OPM-03](organization-project-management-opm03.md) adds guarded project lifecycle/status updates and scope settings persistence with payload-safe audit evidence in the Management view. |
| OPM-04 | P0 | Done | Security Professional + Ops + Developer | Access Inventory And Revocation | [Organization And Project Management OPM-04](organization-project-management-opm04.md) adds payload-safe access inventory, stale-access prompts, and audited revocation for organization/project memberships, role assignments, and namespace grants. |
| OPM-05 | P1 | Done | Product Owner + Knowledge Steward + Developer | Grant Matrix Management | [Organization And Project Management OPM-05](organization-project-management-opm05.md) adds payload-safe role/namespace grant matrix review and replacement using least-privilege presets, effective-access previews, and audited evidence. |
| OPM-06 | P1 | Done | Product Owner + Tester/QA | Management Success Benchmark | [Organization And Project Management OPM-06](organization-project-management-opm06.md) records payload-safe Management evidence for find/inspect duration, safe modification, operation failures, access drift, SQL fallback, raw payload leakage, and Product Owner confidence. |
| OPM-07 | P1 | Done | Product Owner + Security Professional + Developer | Management Activity Timeline | [Organization And Project Management OPM-07](organization-project-management-opm07.md) adds payload-safe organization/project management activity endpoints and UI timeline backed by `access_audit_events` with whitelisted metadata only. |
| OPM-08 | P1 | Done | Product Owner + Knowledge Steward + Security Professional + Developer | Project Role Definition Management | [Organization And Project Management OPM-08](organization-project-management-opm08.md) adds payload-safe custom project role definition list/create/update/disable workflows with dependency guards, audited evidence, and Management UI support. |
...
| PS-03 | P0 | In Progress | Product Owner + Tester/QA + Knowledge Steward + Security Professional + Pilot Operator | Two-Week Pilot Observation | [Project Success Observation PS-03](project-success-observation-ps03.md) starts the observation runbook, keeps live success metrics gated until 2026-06-15, defines two weekly cycles, and points each cycle at the payload-safe `weekly-cycle-template.json` evidence contract. |
| PS-04 | P0 | In Progress | Product Owner + Tester/QA + Knowledge Steward + Security Professional + Release Manager + Pilot Operator | Project Success Closeout | [Project Success Closeout PS-04](project-success-closeout-ps04.md) starts the closeout-prep runbook, keeps the final GO/WATCH/NO-GO recommendation gated until 2026-06-29, and points the closeout at the payload-safe `closeout-template.json` decision record. |
...
Long-Term Memory System internal dogfood pilot and recorded the baseline. PS-03
is in progress as the observation runbook and evidence scaffold; live success
metrics must wait for the 2026-06-15 to 2026-06-28 observation window. PS-04 is
in progress as closeout preparation; the final GO/WATCH/NO-GO recommendation
must wait for the 2026-06-29 to 2026-07-05 closeout window.
...
2026-06-07 update: IP-18 is implemented in [Directory Sync Evaluation
IP-18](directory-sync-evaluation-ip18.md).
`scripts/directory-sync-evaluation.sh` now evaluates pilot pressure signals for
SCIM or group sync while preserving the Decision 0047 boundary: future sync is
provisioning-only and runtime authorization stays with local memberships, role
assignments, namespace grants, effective-access previews, and audit records.
