...
readiness work. The current state is:
...
- EPR-04 is blocked with a formal NO-GO record.
- The first external pilot user must not be invited until a signed GO
...
| Current external-pilot decision | [External Pilot Go/No-Go EPR-04](external-pilot-go-no-go-epr04-2026-06-04.md) |
...
nearby dated updates that point to the current NO-GO state.

P2 now extracts a single release-readiness status contract so docs, admin UI, and
tests do not duplicate EPR gate names, statuses, and caveats. The next
engineering improvement is P3: build the pilot operator cockpit on that contract.
