...
| LMSS-05 | P0 | Next | Implement fact-finding endpoint with evidence and policy metadata. | Agents can query facts and receive authorized claims, source ids, confidence, lifecycle status, contradiction summaries, exclusion summaries, and policy metadata. |
