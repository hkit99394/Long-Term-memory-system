...
variable "certificate_arn" {
  type        = string
  description = "TLS certificate ARN for API ingress. Null keeps TLS attachment out of the current platform slice."
  default     = null
}

variable "api_ingress_cidr_blocks" {
  type        = list(string)
  description = "CIDR blocks allowed to reach the API ingress during pilot."
  default     = []
}
...
variable "postgres_instance_class" {
  type        = string
  description = "Pilot RDS instance class placeholder."
  default     = "db.t4g.micro"
}

variable "postgres_allocated_storage_gib" {
  type        = number
  description = "Initial pilot RDS storage in GiB."
  default     = 20
}

variable "postgres_max_allocated_storage_gib" {
  type        = number
  description = "Maximum autoscaled pilot RDS storage in GiB."
  default     = 100
}

variable "postgres_storage_kms_key_id" {
  type        = string
  description = "Optional KMS key id or ARN for pilot RDS storage encryption."
  default     = null
}

variable "postgres_master_user_secret_kms_key_id" {
  type        = string
  description = "Optional KMS key id or ARN for the pilot RDS-managed master user secret."
  default     = null
}
...
variable "postgres_backup_window" {
  type        = string
  description = "Preferred pilot RDS backup window in UTC."
  default     = "02:00-03:00"
}

variable "postgres_maintenance_window" {
  type        = string
  description = "Preferred pilot RDS maintenance window in UTC."
  default     = "sun:03:00-sun:04:00"
}

variable "deletion_protection" {
  type        = bool
  description = "Whether the pilot database should use deletion protection."
  default     = true
}

variable "postgres_skip_final_snapshot" {
  type        = bool
  description = "Whether pilot RDS deletion can skip the final snapshot."
  default     = false
}

variable "postgres_auto_minor_version_upgrade" {
  type        = bool
  description = "Whether pilot RDS minor PostgreSQL updates can apply automatically."
  default     = true
}

variable "postgres_allow_major_version_upgrade" {
  type        = bool
  description = "Whether pilot RDS major PostgreSQL updates are allowed."
  default     = false
}

variable "postgres_multi_az" {
  type        = bool
  description = "Whether pilot RDS Multi-AZ is enabled."
  default     = false
}

variable "postgres_publicly_accessible" {
  type        = bool
  description = "Whether pilot RDS receives a public endpoint."
  default     = false
}

variable "postgres_apply_immediately" {
  type        = bool
  description = "Whether pilot RDS changes can apply outside the maintenance window."
  default     = false
}

variable "postgres_performance_insights_enabled" {
  type        = bool
  description = "Whether pilot RDS Performance Insights is enabled."
  default     = false
}

variable "postgres_enabled_cloudwatch_logs_exports" {
  type        = list(string)
  description = "Pilot RDS PostgreSQL log exports."
  default     = ["postgresql", "upgrade"]
}

variable "postgres_iam_database_authentication_enabled" {
  type        = bool
  description = "Whether pilot RDS IAM database authentication is enabled."
  default     = false
}

variable "postgres_ca_cert_identifier" {
  type        = string
  description = "Optional pilot RDS CA certificate identifier."
  default     = null
}

variable "database_client_security_group_ids" {
  type        = list(string)
  description = "Security group ids allowed to connect to pilot PostgreSQL."
  default     = []
}

variable "database_client_cidr_blocks" {
  type        = list(string)
  description = "IPv4 CIDR blocks allowed to connect to pilot PostgreSQL."
  default     = []
}
...
variable "backup_export_schedule_expression" {
  type        = string
  description = "Pilot backup export schedule expression."
...
variable "alert_route_test_enabled" {
  type        = bool
  description = "Whether the pilot environment must run the synthetic alert route test before release."
  default     = true
}
